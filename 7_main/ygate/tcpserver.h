...
class EventCenter {
public:
    EventCenter():inQueueName("Queue_Req"),outQueueName("Queue_Ans")
    {
        base = event_base_new();
        if (!base) {
    ...
    void InitNetCenter(int port)
    {
        ...
        onConnection = [&](std::shared_ptr<TCPConnection> conn) {
            ...
            conn->setReadCallback([&](const std::string& data) {
                //std::cout << "Tcp Received data: " << data << std::endl;
                this->amqpChannel->publish("", inQueueName, data.c_str());
            ...
            conn->setEventCallback([&](evutil_socket_t fd, short events) {
                ...
                if (events & BEV_EVENT_EOF) {
                    ...
                    std::cout << "Erase one connection! fd:" <<  fd << ", counts:" << connections.size() << std::endl;
                }
            });
        };
    }
    ...
    void InitAmqpCenter()
    {
        ...
        // 声明队列并设置回调函数
        amqpChannel->declareQueue(inQueueName, AMQP::durable)
            .onSuccess([&]() {
                std::cout << "created queue ok : " << inQueueName << std::endl;
            })
        ...
        amqpChannel->declareQueue(outQueueName, AMQP::durable)
            .onSuccess([&]() {
                std::cout << "created queue ok : " << outQueueName << std::endl;
                // 消费队列中的消息
                amqpChannel->consume(outQueueName)
                    .onReceived([&](const AMQP::Message &message, uint64_t deliveryTag, bool redelivered) {
                        // 获取消息内容
                        std::string msgContent(message.body(), message.bodySize());
                        std::cout << "Received message from queue [" << outQueueName << "]: " << msgContent << std::endl;
                        // 手动确认消息
